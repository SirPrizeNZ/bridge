...
// Assert against the declared version rather than a literal, so a release bump
// cannot leave the suite pinned to a stale number (it was pinned to 0.2.7).
const PKG_VERSION=JSON.parse(readFileSync(new URL('../package.json',import.meta.url),'utf8')).version;
import http from 'node:http';

const PORT=Number(process.env.FIGMA_AGENT_BRIDGE_PORT||3874);
const child=spawn(process.execPath,['bridge/server.mjs'],{stdio:['pipe','pipe','pipe'],env:{...process.env,FIGMA_AGENT_BRIDGE_PORT:String(PORT)}});
...
try{
  ...
  const h=await request('GET','/health'); if(!h.body?.ok||h.body.version!==PKG_VERSION)throw new Error('health/version failed');
  rpc(1,'initialize',{protocolVersion:'2026-07-28'});const init=await waitLine(x=>x.id===1);if(init.result?.serverInfo?.version!==PKG_VERSION)throw new Error('initialize failed');
  ...
  const paired=await request('POST','/pair',{code:st.pairingCode,clientId,installId,plugin:{fileName:'Smoke.fig',bridgeVersion:PKG_VERSION}});if(paired.status!==200||!paired.body?.token)throw new Error('pair failed '+paired.text);const token=paired.body.token;
  ...
  const hello=await request('POST','/hello',{token,clientId,installId,plugin:{fileName:'Smoke.fig',bridgeVersion:PKG_VERSION}});if(hello.status!==200)throw new Error('hello failed '+hello.text);
  ...
  rpc(6,'tools/call',{name:'bridge_doctor',arguments:{}});const doctor=await waitLine(x=>x.id===6);if(!doctor.result?.content?.[0]?.text?.includes(PKG_VERSION))throw new Error('doctor failed');
